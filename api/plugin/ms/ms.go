...
func MS(info *config.HostInfo) error {
...
func msCheck(info *config.HostInfo) error {
	ip := info.Host
	conn, err := xhttp.WrapperTcpWithTimeout("tcp", ip+":445", time.Duration(config.Timeout)*time.Second)
	defer func() {
		if conn != nil {
			err := conn.Close()
			if err != nil {
