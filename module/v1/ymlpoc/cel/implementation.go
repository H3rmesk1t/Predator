	...
	newOptions := []cel.ProgramOption{
		cel.Functions(
			...
			&functions.Overload{
				Operator: "newReverse",
				Function: func(values ...ref.Val) ref.Val {
					return reg.NativeToValue(YmlNewReverse())
				},
			},
		),
	}
...
func YmlNewReverse() *structs.Reverse {
...
func reverseCheck(r *structs.Reverse, timeout int64) bool {
	...
	urlStr := fmt.Sprintf("http://api.ceye.io/v1/records?token=%s&type=dns&filter=%s", config.CeyeApi, sub)
	req, _ := xhttp.NewRequest("GET", urlStr, nil)
	resp, _, err := lib.DoRequest(req, false)
	if err != nil {
		return false
	}
